...
class MemoryStore {
  ...
  linkMemories(opts = {}) {
    ...
    const fromMemory = this._memories.get(fromMemoryId);
    const toMemory = this._memories.get(toMemoryId);
    ...
    if (this._db) {
      ...
      try {
        ...
        return { ok: false, error: { code: 'DATABASE_ERROR', message: err.message } };
      }
    }
  ...
  queryLinks(opts = {}) {
    ...
    for (const link of this._links) {
      ...
      if (!includeDeleted) {
        const fromMem = this._memories.get(link.fromMemoryId);
        const toMem = this._memories.get(link.toMemoryId);
  ...
  eventsForMemory(memoryId, opts = {}) {
    ...
    const record = this._memories.get(id);
