...
const DEFAULT_MEMORY_LIMITS = Object.freeze({
  maxImportMemories: 1000,
  maxImportEvents: 5000,
  maxImportLinks: 5000,
  maxMemoryContentBytes: 64000,
  maxTitleSummaryBytes: 4000,
  maxMetadataJsonBytes: 32000,
  maxQueryTextBytes: 2000,
  defaultListLimit: 500,
  defaultQueryLimit: 100,
  defaultLinkLimit: 100,
  defaultEventsForMemoryLimit: 100,
...
function getStableJsonSize(value) {
  return Buffer.byteLength(toStableString(value), 'utf8');
}

function validateFiniteNonNegativeInteger(value, fieldName) {
  const num = Number(value);
  if (!Number.isFinite(num) || num < 0) {
    return { ok: false, error: { code: 'VALIDATION_ERROR', message: `${fieldName} must be a finite non-negative number` } };
  }
...
class MemoryStore {
  constructor(opts = {}) {
    ...
    this._limits = DEFAULT_MEMORY_LIMITS;
  ...
  _normalizeLimit(rawLimit, fieldName, defaultLimit) {
    const maxLimit = this._limits.maxResultLimit;
    if (rawLimit === undefined) {
      return { ok: true, limit: defaultLimit };
    }
    if (rawLimit === null) {
      return { ok: true, limit: maxLimit };
    }
    const parsed = validateFiniteNonNegativeInteger(rawLimit, fieldName);
    if (!parsed.ok) return parsed;
    if (parsed.value > maxLimit) {
      return {
        ok: false,
        error: {
          code: 'VALIDATION_ERROR',
          message: `${fieldName} exceeds max limit of ${maxLimit}`,
        },
      };
    }
    return { ok: true, limit: parsed.value };
  }

  _normalizeOffset(rawOffset) {
    if (rawOffset === undefined) {
      return { ok: true, offset: 0 };
    }
    const parsed = validateFiniteNonNegativeInteger(rawOffset, 'offset');
    if (!parsed.ok) return parsed;
    return { ok: true, offset: parsed.value };
  }

  _validateTextFilterLength(value, fieldName) {
    if (value === undefined || value === null) return { ok: true };
    if (getUtf8Size(value) > this._limits.maxQueryTextBytes) {
      return {
        ok: false,
        error: {
          code: 'VALIDATION_ERROR',
          message: `${fieldName} exceeds max length of ${this._limits.maxQueryTextBytes} bytes`,
        },
      };
    }
    return { ok: true };
  ...
  _validateContentAndMetadataLimits(content, metadata, fieldPrefix = 'memory') {
    if (content !== undefined && content !== null) {
      if (getStableJsonSize(content) > this._limits.maxMemoryContentBytes) {
        return {
          ok: false,
          error: {
            code: 'VALIDATION_ERROR',
            message: `${fieldPrefix} content exceeds max size of ${this._limits.maxMemoryContentBytes} bytes`,
          },
        };
      }
      if (typeof content === 'object' && content !== null) {
        for (const key of ['title', 'summary']) {
          if (typeof content[key] === 'string' && getUtf8Size(content[key]) > this._limits.maxTitleSummaryBytes) {
            return {
              ok: false,
              error: {
                code: 'VALIDATION_ERROR',
                message: `${fieldPrefix} ${key} exceeds max size of ${this._limits.maxTitleSummaryBytes} bytes`,
              },
            };
          }
        }
    ...
    if (metadata !== undefined && metadata !== null && getStableJsonSize(metadata) > this._limits.maxMetadataJsonBytes) {
      return {
        ok: false,
        error: {
          code: 'VALIDATION_ERROR',
          message: `${fieldPrefix} metadata exceeds max size of ${this._limits.maxMetadataJsonBytes} bytes`,
        },
      };
    }
    return { ok: true };
  ...
  _validateImportPackageLimits(pkg) {
    if (pkg.memories.length > this._limits.maxImportMemories) {
      return {
        ok: false,
        error: {
          code: 'INVALID_PACKAGE',
          message: `package memories exceed max limit of ${this._limits.maxImportMemories}`,
        },
      };
    }
    if (pkg.events.length > this._limits.maxImportEvents) {
      return {
        ok: false,
        error: {
          code: 'INVALID_PACKAGE',
          message: `package events exceed max limit of ${this._limits.maxImportEvents}`,
        },
      };
    }
    if (pkg.links.length > this._limits.maxImportLinks) {
      return {
        ok: false,
        error: {
          code: 'INVALID_PACKAGE',
          message: `package links exceed max limit of ${this._limits.maxImportLinks}`,
        },
      };
    }
    return { ok: true };
  ...
  store(input = {}) {
    ...
    const payloadValidation = this._validateContentAndMetadataLimits(input.content, input.metadata || {});
  ...
  list(opts = {}) {
    ...
    const limitValidation = this._normalizeLimit(opts.limit, 'limit', this._limits.defaultListLimit);
    if (!limitValidation.ok) {
      return { ok: false, error: limitValidation.error };
    }
    const offsetValidation = this._normalizeOffset(opts.offset);
    if (!offsetValidation.ok) {
      return { ok: false, error: offsetValidation.error };
    }
    ...
    return { ok: true, memories: results, total, limit, offset };
  }
  ...
  patchMetadata(memoryId, patch = {}, opts = {}) {
    ...
    const metadataValidation = this._validateContentAndMetadataLimits(undefined, nextMetadata);
  ...
  supersede(oldMemoryId, newContent, opts = {}) {
    ...
    const contentValidation = this._validateContentAndMetadataLimits(newContent, opts.metadata || {});
   ...
   * @returns {object[]}
   */
  getEvents(memoryId, opts = {}) {
    if (!memoryId) return [];
    const id = String(memoryId).trim();
    const workspaceId = normalizeWorkspaceId(opts.workspaceId);
    return this._events.filter(e => e.workspaceId === workspaceId && e.memoryId === id);
  }
  ...
  getLinks(memoryId, opts = {}) {
    ...
    const id = String(memoryId).trim();
    const workspaceId = normalizeWorkspaceId(opts.workspaceId);
    return this._links.filter(l => l.workspaceId === workspaceId && (l.fromMemoryId === id || l.toMemoryId === id));
  }
  ...
  query(opts = {}) {
    ...
    const limitValidation = this._normalizeLimit(opts.limit, 'limit', this._limits.defaultQueryLimit);
    if (!limitValidation.ok) {
      return { ok: false, error: limitValidation.error };
    }
    const offset = offsetValidation.offset;
    ...
    const textValidation = this._validateTextFilterLength(contentIncludes, 'query text');
    if (!textValidation.ok) {
      return { ok: false, error: textValidation.error };
    }
  ...
  linkMemories(opts = {}) {
    ...
    const metadataValidation = this._validateContentAndMetadataLimits(undefined, opts.metadata || {}, 'link');
    if (!metadataValidation.ok) {
      return { ok: false, error: metadataValidation.error };
    }
  ...
  queryLinks(opts = {}) {
    ...
    const limitValidation = this._normalizeLimit(opts.limit, 'limit', this._limits.defaultLinkLimit);
    if (!limitValidation.ok) {
      return { ok: false, error: limitValidation.error };
    }
    const offset = offsetValidation.offset;
  ...
  eventsForMemory(memoryId, opts = {}) {
    ...
    const limitValidation = this._normalizeLimit(opts.limit, 'limit', this._limits.defaultEventsForMemoryLimit);
    if (!limitValidation.ok) {
      return { ok: false, error: limitValidation.error };
    }
    const offset = offsetValidation.offset;
  ...
  timeline(opts = {}) {
    ...
    const limitValidation = this._normalizeLimit(opts.limit, 'limit', this._limits.defaultTimelineLimit);
    if (!limitValidation.ok) {
      return { ok: false, error: limitValidation.error };
    }
    const offset = offsetValidation.offset;
  ...
  exportPackage(opts = {}) {
    ...
    if (
      memories.length > this._limits.maxExportRecords ||
      events.length > this._limits.maxExportRecords ||
      links.length > this._limits.maxExportRecords
    ) {
      return {
        ok: false,
        error: {
          code: 'VALIDATION_ERROR',
          message: `export exceeds max record limit of ${this._limits.maxExportRecords} per collection`,
        },
      };
    }
  ...
  importPackage(pkg, opts = {}) {
    ...
    const packageLimitValidation = this._validateImportPackageLimits(pkg);
    if (!packageLimitValidation.ok) {
      return { ok: false, error: packageLimitValidation.error };
    }
    ...
    try {
      this._withTransaction(() => {
        for (const mem of pkg.memories) {
          ...
          const payloadValidation = this._validateContentAndMetadataLimits(normalized.content, normalized.metadata);
          if (!payloadValidation.ok) {
            conflicts.push({ type: 'memory', memoryId: mem.memoryId, reason: payloadValidation.error.message });
            continue;
          }
        ...
        for (const evt of pkg.events) {
          ...
          const detailsValidation = this._validateContentAndMetadataLimits(undefined, normalizedEvt.details || {}, 'event');
          if (!detailsValidation.ok) {
            conflicts.push({ type: 'event', eventId: evt.eventId, reason: detailsValidation.error.message });
            continue;
          }
        ...
        for (const lnk of pkg.links) {
          ...
          const linkMetadataValidation = this._validateContentAndMetadataLimits(undefined, normalizedLnk.metadata || {}, 'link');
          if (!linkMetadataValidation.ok) {
            conflicts.push({ type: 'link', linkId: lnk.linkId, reason: linkMetadataValidation.error.message });
            continue;
          }
