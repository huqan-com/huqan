...
function detectPredicateDrift(stored, incoming, opts = {}) {
  ...
  const storedWords = tokenize(parts.storedText);
  const incomingWords = tokenize(parts.incomingText);
  const overlap = storedWords.filter(word => incomingWords.includes(word));
  const relationMismatch = Boolean(parts.storedRelation && parts.incomingRelation && parts.storedRelation !== parts.incomingRelation);
  if (!relationMismatch && overlap.length < 2 && !hasMeaningfulOverlap(parts.storedText, parts.incomingText, 2)) return null;
  ...
  return {
    rule: CONTRADICTION_RULES.PREDICATE_DRIFT,
    kind: 'contradiction',
    severity: 0.55,
    confidence: 0.6,
    flags: [CONTRADICTION_RULES.PREDICATE_DRIFT, 'RELATION_DRIFT'],
