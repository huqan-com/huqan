...
const { detectAbsoluteClaim, detectHighRiskDomain, detectMultilingualAmbiguity, detectWeakPartialMatch } = require('./risk-rules');
...
class VerifyService {
  ...
  verify(statement, opts = {}) {
    ...
    if (numericComparison) {
      return this._verifyResult(statement, opts, {
        ...
        text: `SayÄ±sal karÅŸÄ±laÅŸtÄ±rma: "${numericComparison.left} ${numericComparison.operator} ${numericComparison.right}"`,
        confidence: 0.98,
    ...
    if (parts.length < 2) {
      return this._verifyResult(statement, opts, { status: 'bilinmiyor', confidence: 0 }, [], { workspaceId });
    }

    const subject = this.kernel.normalizeWord(parts[0]);
    const subjectNode = this.kernel.graph.getNode(subject, workspaceId);
    if (!subjectNode) {
      return this._verifyResult(statement, opts, { status: 'bilinmiyor', confidence: 0 }, [], { workspaceId, subject });
    }

    const edges = this.kernel.graph.getEdges(subject, workspaceId);
    ...
    if (predicateNumericComparison) {
      return this._verifyResult(statement, opts, {
        ...
        text: `SayÄ±sal karÅŸÄ±laÅŸtÄ±rma: "${predicateNumericComparison.left} ${predicateNumericComparison.operator} ${predicateNumericComparison.right}"`,
        confidence: 0.95,
        nodes: [subject, String(predicateNumericComparison.left), String(predicateNumericComparison.right)],
        edges: [],
      }], { workspaceId, subject, predicate, edges });
    }

    const negMatch = predicate.match(/^(.*?)\s+(de[ÄŸg]il|de[ÄŸg]ildir|not)\s*$/i);
    if (negMatch) {
    ...
    if (directEdge) {
      ...
      return this._verifyResult(statement, opts, { status: 'dogrulandi', confidence }, [this.kernel._edgeEvidence(directEdge, 'direct_edge', confidence)], { workspaceId, subject, predicate, edges, directEdge });
      }
    }
    ...
    if (subjCons.length > 0) {
      ...
      return this._verifyResult(statement, opts, { status: 'celiski', confidence: 0.7 }, evidence, { workspaceId, subject, predicate, edges });
    }
    ...
    if (target !== subject) {
      ...
      if (pathResult.path && !incomingAbsolute) {
        return this._verifyResult(statement, opts, { status: 'dogrulandi', confidence: 0.5 }, [this.kernel._pathEvidence(pathResult.path, 'path', 0.5)], { workspaceId, subject, predicate, edges, pathSearch: pathResult });
      }
    }
          ...
          if (mismatch) {
            ...
            if (hasTextOverlap) {
              return this._verifyResult(statement, opts, { status: 'celiski', confidence: 0.75 }, [{
                kind: 'contradiction',
                text: `SayÄ±sal Ã§eliÅŸki: "${predicate}" ifadesinde ${stmtNums.join(',')} ama "${edge.to}" bilgisinde ${edgeNums.join(',')}`,
                confidence: 0.75,
    ...
    for (const word of parts.slice(1)) {
      ...
      if (match) {
        ...
        if (contradictionSignals.length > 0) {
          ...
          return this._verifyResult(statement, opts, { status: 'celiski', confidence: 0.75 }, evidence, { workspaceId, subject, predicate, edges, fuzzy });
        }
        if (incomingAbsolute || fuzzy.isWeak) continue;
        return this._verifyResult(statement, opts, { status: 'dogrulandi', confidence: 0.35 }, [this.kernel._edgeEvidence(match, 'partial_match', 0.35)], { workspaceId, subject, predicate, edges, fuzzy });
      }
    }

    return this._verifyResult(statement, opts, { status: 'bilinmiyor', confidence: 0 }, [], { workspaceId, subject, predicate, edges });
  }
  ...
  _verifyResult(statement, opts, data, evidence, context = {}) {
    ...
    return this.kernel._ok('verify', nextData, evidence, { semanticTrust });
  }
  ...
  detectContradictions(subject = '', workspaceId = 'default') {
    ...
    for (const node of allNodes) {
      ...
      const typeEdges = edges.filter(e => e.relation === 'tÃ¼r');
    ...
    for (const node of allNodes) {
      ...
      for (const edge of nodeEdges) {
        if (edge.relation !== 'tÃ¼r') continue;
        const backEdge = this.kernel.graph.getEdge(edge.to, node.id, 'tÃ¼r', scope);
        if (backEdge && !contradictions.some(c => c.type === 'dÃ¶ngÃ¼' && c.node === node.id)) {
          contradictions.push({
            type: 'dÃ¶ngÃ¼',
            node: node.id,
            targets: [edge.to],
    ...
    for (const node of allNodes) {
      const edges = this.kernel.graph.getEdges(node.id, scope);
      const degilEdges = edges.filter(e => e.relation === 'deÄŸil');
      if (degilEdges.length === 0) continue;
      const otherEdges = edges.filter(e => e.relation !== 'deÄŸil' && e.relation !== 'benzer' && e.relation !== 'hipotez');
      for (const degil of degilEdges) {
        const degilCore = degil.to.replace(/(?:maz|mez|mamak|memek|deÄŸildir|deÄŸil)$/i, '').trim();
        for (const other of otherEdges) {
          const otherCore = other.to.replace(/(?:maz|mez|mamak|memek|deÄŸildir|deÄŸil|yapabilir|yapamaz|edebilir|edemez)$/i, '').trim();
  ...
  _contradictionEvidence(contradiction) {
    ...
    const edges = Array.isArray(contradiction.edges)
      ...
      : targets.map(to => ({ from: contradiction.node, to, relation: contradiction.relation || 'tÃ¼r' }));
    return {
      kind: 'contradiction',
