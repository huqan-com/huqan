...
function queryTrustGraph(target, filters = {}) {
  ...
  const receipt = normalizeTrustReceipt({
    ...
    targetType: canonical?.targetType || (filters.candidateId ? 'candidate_claim' : filters.targetType || ''),
    targetId: canonical?.targetId || filters.targetId || filters.candidateId || filters.sourceRef || filters.provenanceId || '',
    claim: canonical?.claim || candidateClaims[0]?.claim || filters.claim || '',
    status,
    workspaceId,
    provenance: canonical?.provenance || provenance[0]?.provenance || candidateClaims[0]?.provenance || null,
    trustPolicyVersion: canonical?.trustPolicyVersion || provenance[0]?.trustPolicyVersion || candidateClaims[0]?.trustPolicyVersion || '',
    confidence: canonical?.confidence ?? provenance[0]?.confidence ?? candidateClaims[0]?.confidence ?? 0.5,
    auditTrail,
    conflict: conflict?.conflict || null,
    candidateClaim: candidateClaims[0] || null,
