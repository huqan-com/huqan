...
| `company-brain.js` entity scan | all-node object values | Read | Explicit-workspace filtering when supplied, otherwise all-workspace scan and ordering | `getNodes(workspaceId)` is equivalent only for the explicit-workspace branch | Reuse the existing method only in that branch; the all-workspace branch still needs contract evidence |
